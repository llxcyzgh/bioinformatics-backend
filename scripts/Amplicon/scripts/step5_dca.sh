#!/bin/bash
# step5_dca.sh - DCA 去趋势对应分析脚本
# 用法：bash step5_dca.sh -t <asv_table.even.txt> -g <group.list>
...
# Perl 脚本路径
PLOT_DCA_PL="${PLOT_DCA_PL:-/newVol/users/guorongjun/work/Pipline/Amplicon_pipeline/Amplicon_pipeline_V1.0/lib/05.Diversity_analysis/lib/DCA/plot_dca.pl}"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            echo "用法：bash step5_dca.sh -t <asv_table.even.txt> -g <group.list>"
            echo ""
            echo "参数:"
...
# Step 1: 绘制 DCA 排序图
echo "[1/2] 绘制 DCA 排序图..."
"${PERL_BIN}" "${PLOT_DCA_PL}" "${ASV_TABLE}" "${GROUP_FILE}"

# Step 2: PDF → PNG 转换
echo "[2/2] 转换 PDF → PNG..."
"${CONVERT_BIN}" -density 300 DCA.pdf DCA.png 2>/dev/null || true

echo ""
echo "✅ DCA 去趋势对应分析完成"
echo ""
echo "📊 输出文件:"
echo "   - DCA.svg: DCA 排序图（SVG 格式）"
echo "   - DCA.pdf: DCA 排序图（PDF 格式）"
echo "   - DCA.png: DCA 排序图（PNG 格式）"
echo ""
