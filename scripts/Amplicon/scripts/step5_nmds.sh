...
# 用法：bash step5_nmds.sh -i <asv_table.even.txt|Relative/|*.relative.xls> -g <group.list> [-l <levels>] [-c <group_col.list>] -o <output_dir>
...
for tool in STAT_OTU_TAB_PL NMDS_R_PL COLOR_DEFINED_PL; do
    if [ ! -f "${!tool}" ]; then
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            echo "用法：bash step5_nmds.sh -i <input> -g <group.list> [-l <levels>] [-c <group_col.list>] -o <output_dir>"
            echo ""
            echo "参数:"
            ...
            echo "  -c, --color      分组颜色配置文件（可选，默认由 group.list 自动生成）"
            echo "  -o, --output     输出目录"
            echo "  -h, --help       显示帮助"
            echo ""
...
COLOR_FILE="$(abs_path_file_optional "${COLOR_FILE}")"
...
if [ -z "${COLOR_FILE}" ]; then
    ...
    COLOR_FILE="${OUTPUT_DIR}/${GROUP_COL_LIST}"
    GENERATED_COLOR=1
...
for level_name in "${SELECTED_LEVELS[@]}"; do
    ...
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "🔬 NMDS 分析：${level_name} 水平"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "📊 相对丰度表：${rel_table}"

    echo "[1/2] 绘制 NMDS 排序图..."
    "${PERL_BIN}" "${NMDS_R_PL}" "${rel_table}" "${GROUP_FILE}" "${COLOR_FILE}"
    ...
    if [ "${multi_level}" -eq 1 ]; then
        for ext in pdf png svg; do
            [ -f "NMDS.${ext}" ] && mv -f "NMDS.${ext}" "NMDS.${level}.${ext}"
            [ -f "NMDS_2.${ext}" ] && mv -f "NMDS_2.${ext}" "NMDS.${level}_2.${ext}"
            [ -f "NMDS_withcluster.${ext}" ] && mv -f "NMDS_withcluster.${ext}" "NMDS.${level}_withcluster.${ext}"
        done
        echo "  ✅ ${level_name} 水平完成：NMDS.${level}.pdf/png"
    else
        echo "  ✅ ${level_name} 水平完成：NMDS.pdf/png"
    fi
    echo ""
...
for level_name in "${SELECTED_LEVELS[@]}"; do
    level="${LEVEL_TO_CODE[$level_name]}"
    if [ "${multi_level}" -eq 1 ]; then
        echo "   - NMDS.${level}.png/pdf: ${level_name} 水平 NMDS 排序图"
    else
        echo "   - NMDS.png/pdf: ${level_name} 水平 NMDS 排序图"
    fi
    [ "${INPUT_MODE}" = "relative_file" ] && break
done
