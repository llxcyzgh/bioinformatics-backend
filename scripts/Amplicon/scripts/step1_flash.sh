...
# 用法：bash step1_flash.sh -1 <sample_R1.fastq.gz> -2 <sample_R2.fastq.gz> [-m min_overlap] [-M max_overlap] [-x max_ratio] [-t threads]
...
SAMPLE_NAME=$(basename "${R1_FASTQ}" | sed 's/_R1\.fastq\.gz$//')
...
FLASH_LOG="${SAMPLE_NAME}.flash.log"
...
echo ""
echo "📊 输出文件:"
echo "   - ${OUTPUT_PREFIX}.out.extendedFrags.fastq: 拼接后的序列"
