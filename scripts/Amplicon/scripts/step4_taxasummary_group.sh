...
# 用法：bash step4_taxasummary_group.sh -i <Relative_group/> [--top <N>]
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --top) TOP_N="$2"; shift 2 ;;
        -h|--help)
            echo "用法：bash step4_taxasummary_group.sh -i <Relative_group/> [--top <N>]"
            echo ""
            echo "参数:"
...
for i in "${!LEVELS[@]}"; do
    ...
    "${PYTHON_BIN}" "${HEATMAP_DATA_PY}" \
        --relative "${INDIR}/asv_table_group.${level}.relative.xls" \
        --top "${TOP_N}" \
        --level "${level}" \
        --outdir cluster
...
echo ""
echo "📊 输出文件:"
echo "   - cluster/cluster.p.png/pdf: 门水平分组热图"
echo "   - cluster/cluster.c.png/pdf: 纲水平分组热图"
echo "   - cluster/cluster.o.png/pdf: 目水平分组热图"
echo "   - cluster/cluster.f.png/pdf: 科水平分组热图"
echo "   - cluster/cluster.g.png/pdf: 属水平分组热图"
echo "   - cluster/cluster.s.png/pdf: 种水平分组热图"
echo ""
