...
|--------|------|------|----------|----------|----------|
| manifest.tsv | 样本与序列文件路径的对应关系表 | TSV | 16S/18S/ITS | 通用 | (${AMPLICON_ROOT}/v2/examples/step3_dada2/manifest.tsv) |
| ${sample}.fastq | 质控后的序列文件（由 manifest 引用） | FASTQ | 16S/18S/ITS | 通用 | (${AMPLICON_ROOT}/v2/examples/step3_dada2/T1001.fastq) |
...
**通用格式**：
```bash
bash ${AMPLICON_ROOT}/v2/scripts/step3_dada2.sh -m manifest.tsv -t <trunc_len> -n <threads> -o DADA2_Output/
# 输出目录：DADA2_Output/；文件：featureSeqs.qza, feature-table.tsv, feature.fasta, featureTable.qza, featureTable.biom
```

**示例**：
```bash
bash ${AMPLICON_ROOT}/v2/scripts/step3_dada2.sh -m manifest.tsv -t 0 -n 12 -o DADA2_Output/
# 输出目录：DADA2_Output/；文件：featureSeqs.qza, feature-table.tsv, feature.fasta, featureTable.qza, featureTable.biom
```
...
source .env
bash step3_dada2.sh ... -o DADA2_Output/
```
...
- **脚本位置**: `${AMPLICON_ROOT}/v2/scripts/step3_dada2.sh`
- **参考文档**: `${AMPLICON_ROOT}/v2/reference/step3_dada2.md`
- **示例数据**: `${AMPLICON_ROOT}/v2/examples/step3_dada2/`
